...
export class Dashboard extends React.Component {
  ...
  render() {
      ...
      if (val) {
        return (
          <Card
            key={val._id}
            onPress={() =>
              this.props.navigation.navigate("IndividualCampaign", {
                campaign_Id: val._id
              })
            }
          >
            ...
            <Card.Actions style={{ flex: 1 }}>
              <Button
                onPress={() =>
                  this.props.subscribeList.includes(val._id)
                    ? this.props.startRemoveSubscribe(val._id)
                    : this.props.startAddSubscribe(val._id)
                }
                icon={<Icon name="payment" size={30} color="white" />}
                title="SUBSCRIBE"
                titleStyle={{ fontWeight: "700" }}
                buttonStyle={{
                  backgroundColor: this.props.subscribeList.includes(val._id)
                    ? "#27ae60"
                    : "#e67e22",
                  width: 300,
                  height: 45,
    ...
    let campaignCard = isSubscribe => {
      return this.props.campaign.length > 0 ? (
        this.props.campaign.map(val => {
          if (isSubscribe) {
      ...
      <View style={{ flex: 1, backgroundColor: "#eff4ff" }}>
        <ScrollView style={{ flex: 1 }}>
          <View style={{ alignItems: "center", marginTop: "20%" }}>
            <Text h1 style={{ color: "#3498db" }}>
              Hi {this.props.auth.info.name.split(" ")[0]}
            </Text>
          ...
          <View style={{ alignItems: "center" }}>
            <Avatar
              rounded
              size="xlarge"
              source={{ uri: this.props.auth.info.picture.data.url }}
            />
            <Button
              onPress={() => {
                this.props.navigation.navigate("MyCampaignDashboard");
              }}
              title="MY CAMPAIGN"
              containerStyle={{ marginVertical: 20, marginTop: 20 }}
            ...
            <Text h3>
              {" "}
              Total Spare: $ {this.props.totalSpare.totalSpare.toFixed(2)}
            </Text>
          </View>
          <View>
            <Button
        ...
        <ActionButton
          ...
          onPress={() => {
            this.props.navigation.navigate("CampaignForm");
          }}
        />
      </View>
    );
