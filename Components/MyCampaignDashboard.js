      ...
      <View style={{flex:1, backgroundColor:"#eff4ff"}}>
        ...
        <ScrollView style={{flex:1}}>
            ...
            <View style={{flex:1, justifyContent:'center',alignItems:'center', marginTop:'40%'}}>
                <Text h1 style={{color:'#3498db', textAlign:'center'}}>Managed Campaign</Text>
                {/* <Button title="setSub" onPress={()=>this.props.startSetSubscribe()}></Button> */}
            </View>
            {this.props.campaign.filter((val)=> val._user == this.props.auth.info._id).map((val)=>(
                <View style={{flex:1}}>              
                <Card style={{marginTop:10}}>
                   
                    <Progress.Bar width={Dimensions.get('window').width - 20} progress={val.moneyData.reduce((acc,val)=> acc + val)/2899 }/>
                    
                    <Card.Content numberOfLines={3}>
                    <Title>{val.Title}</Title>
                    <Paragraph>Raised: ${val.moneyData.reduce((acc,val)=> acc + val) || 0.00}</Paragraph>
                    </Card.Content>
                </Card>
                </View>
                ))}
...
const mapStateToProps = (state,props) => {
  ...
  export default connect(mapStateToProps,mapDispatchToProps)(MyCampaignDashboard);