...
export class IndividualCampaign extends React.Component {
    ...
    render() {
        ...
        console.log(this.props.campaign.filter((val)=>val._id == campaignId)[0].moneyData)
      return ( campaignId != 'NO-ID' ?

        <ScrollView style={{flex:1}}>
            <View style={{flex:1, justifyContent:'center',alignContent:'center',alignItem:'center', marginTop:20 }}>
            <Card style={{marginTop:10, elevation:0}}>
                <Card.Cover source={{ uri: this.props.campaign.filter((val)=>val._id == campaignId)[0].ImageURL }} />
                <Card.Content >
                <Title>{this.props.campaign.filter((val)=>val._id == campaignId)[0].Title}</Title>
                <Paragraph>{this.props.campaign.filter((val)=>val._id == campaignId)[0].Description}</Paragraph>
                </Card.Content>
            </Card>
            </View>
            <View style={{flex:1, justifyContent:'center',alignContent:'center', marginTop:20 }}>
            <Text h3> Weekly Performance</Text>
            <Text h4> Total Raised: ${this.props.campaign.filter((val)=>val._id == campaignId)[0].moneyData.reduce((acc,val)=>acc+val)}</Text>
            <AreaChartComponent data = {this.props.campaign.filter((val)=>val._id == campaignId)[0].moneyData}/>
  ...
  export default connect(mapStateToProps,mapDispatchToProps)(IndividualCampaign);
